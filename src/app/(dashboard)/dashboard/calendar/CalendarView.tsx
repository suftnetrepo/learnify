...
import { ChevronLeft, ChevronRight, Video, MapPin, CalendarDays } from "lucide-react";
import { cn } from "@/lib/utils";
import { CourseDetailsDrawer } from "../my-courses/CourseDetailsDrawer";
import type { StudentSession, EnrolledCourse } from "../my-courses/MyCoursesList";
...
function sessionStatus(s: StudentSession, now: Date): "live" | "past" | "upcoming" {
...
export function CalendarView({ enrolled, sessions }: Props) {
  ...
  const selectedEnrollment = selected
    ? (enrolled.find((e) => e.courseId === selected.courseId) ?? null)
    : null;
  ...
  return (
    <div className="premium-calendar rounded-2xl border border-surface-100 bg-white shadow-card overflow-hidden">
      ...
      {selected && selectedEnrollment && (
        <CourseDetailsDrawer
          enrollment={{ ...selectedEnrollment, progress: Number(selectedEnrollment.progress) }}
          session={selected}
